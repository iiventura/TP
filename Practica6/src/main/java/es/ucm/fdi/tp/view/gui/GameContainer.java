...
public class GameContainer<S extends GameState<S,A>,A extends GameAction<S,A>> extends GUIComponents<S,A>implements GameObserver<S,A> {
	...
	private SmartPanel<S,A> smartPanel;
	...
	public GameContainer(String title,GameController<S, A> gameCtrl,
			GameObservable<S, A> game,GUIComponents<S, A> gameView,SmartThread<S,A> thread){
		super(title);
	...
	private void initGUI(){
		...
		smartPanel=new SmartPanel<S,A>();
		
		//Se le asigna el JTextArea al hilo
		thread.setMessageViewer(messageViewer);
		//Se le asigna el hilo al SmartPanel
		smartPanel.setThread(thread);
		
		gameView.setMessageViewer(messageViewer);
		gameView.setPlayersInfoViewer(playersInfoViewer);
		
		
		controlPanel.setSmartPanel(smartPanel);
