			...
			SwingUtilities.invokeAndWait(new Runnable() {
				...
				public void run() {
					...
					for (int i = (game.getState().getPlayerCount()-1);i>=0 ; i--) {
						...
						p1.join(i);
						GamePlayer p2=new SmartPlayer("Player 2",5);
						p2.join(i);	
						GUIComponents<S, A> gView = createGameView(gType,game);
						GameController<S,A> gmCtrl=new GUIController<S,A>(i,p1,p2,game);
						GUIComponents<S,A> container=new GameContainer<S,A>(gType +" Player "+(i),gmCtrl,game,gView);
