...
public class GUIController<S extends GameState<S,A>, A extends GameAction<S,A>> implements GameController<S,A> {
	 ...
	 * @param game juego
	 */
	public GUIController(int playerId, GamePlayer randPlayer, SmartThread<S,A> thread, GameTable<S,A> game){
	...
	public void makeSmartMove() {
		if(getPlayerId()==game.getState().getTurn()&&!game.getState().isFinished()){	
			thread.start();
		}
	}
